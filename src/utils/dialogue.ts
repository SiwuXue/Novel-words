...
const CN_VERB_LIST = [
  ...
  '插嘴道',
  '接着说',
  // 真实文本常见：动词前有逗号时靠这些"动作+道"组合吃掉动作词
  '笑骂道',
  '嗤笑道',
  '微笑道',
  '含笑道',
...
/** 动词集合（O(1) 判定）：用于识别「动作残片」（如候选「感慨」+「道」=「感慨道」是动词）。 */
const CN_VERB_SET: ReadonlySet<string> = new Set(CN_VERB_LIST)
const EN_VERBS = [
  'whispered',
  'exclaimed',
  'shouted',
...
/**
 * 动作/神态残片黑名单（真实语料实证：「……，说道：」这类句子名字被逗号断开时，
 * 正则只能退到单字动词「道」，于是把动词本身当成了说话人）。
 */
const ACTION_RESIDUE = new Set([
  '起身',
  '失色',
  '急忙',
  '诚恳',
...
function cleanName(raw: string): string | null {
  ...
  if (!ascii) {
    // 单字候选一律不是人名：真实文本里它们是「…，说道：」退化成单字动词后的残片（说/笑/喝）
    if (name.length === 1) return null
    // 动词残片：候选本身是动词（感慨/打趣/傲然 + 道|说），或落在动作词黑名单
    if (
      CN_VERB_SET.has(name) ||
      CN_VERB_SET.has(`${name}道`) ||
      CN_VERB_SET.has(`${name}说`) ||
      ACTION_RESIDUE.has(name)
    ) {
      return null
    }
    // 短神态词（冷声/怒声/失色/正色）
    if (name.length <= 3 && /[声色]$/.test(name)) return null
  }
